...
from capabilities import clients
...
def generate_code(pattern: str = CODE_BLOCK_PATTERN, **config) -> Tuple[str, float]:
    ...
    """
    response = clients.Completion.create(**config)
    return extract_code(clients.Completion.extract_text(response)[0], pattern), response["cost"]
...
def improve_function(file_name, func_name, objective, **config):
    ...
    with open(file_name, "r") as f:
        file_string = f.read()
    response = clients.Completion.create(
        {"func_name": func_name, "objective": objective, "file_string": file_string}, **params
    )
    return clients.Completion.extract_text(response)[0], response["cost"]
...
def get_powershell_command():
    try:
        result = subprocess.run(
            ["powershell", "$PSVersionTable.PSVersion.Major"], capture_output=True, text=True
        )
