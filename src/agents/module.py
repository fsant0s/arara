...
@dataclass
class Module:
    """(In preview) A Module data class that contains the following data fields:
    ...
    - speaker_selection_method: the method for selecting the next speaker. Default is "auto".
        ...
        - a customized speaker selection function (Callable): the function will be called to select the next speaker.
            The function should take the last speaker and the module as input and return one of the following:
                1. an `Agent` class, it must be one of the agents in the module.
                2. a string from ['auto', 'manual', 'random', 'round_robin'] to select a default method to use.
                3. None, which would terminate the conversation gracefully.
            ```python
            def custom_speaker_selection_func(
                last_speaker: Agent, module: Module
    ...
    - enable_clear_history: enable possibility to clear history of messages for agents manually by providing
        ...
        See description of Orchestrator.clear_agents_history function for more info.
    ...
    def __post_init__(self):
        ...
        if self.speaker_transitions_type not in self._VALID_SPEAKER_TRANSITIONS_TYPE:
            raise ValueError(
                f"Module speaker_transitions_type is set to '{self.speaker_transitions_type}'. "
    ...
    def agent_names(self) -> List[str]:
        """Return the names of the agents in the module."""
    ...
    def append(self, message: Dict, speaker: Agent):
        """Append a message to the module.
    ...
    def _prepare_and_select_agents(
        ...
        if isinstance(self.speaker_selection_method, Callable):
            ...
            if selected_agent is None:
                ...
                if selected_agent in self.agents:
                    ...
                    raise ValueError(
                        f"Custom speaker selection function returned an agent {selected_agent.name} not in the module."
                    )
        ...
        if speaker_selection_method.lower() not in self._VALID_SPEAKER_SELECTION_METHODS:
            raise ValueError(
                f"Module speaker_selection_method is set to '{speaker_selection_method}'. "
        ...
        agents = self.agents
        n_agents = len(agents)
        # Warn if Module is underpopulated
        if n_agents < 2:
            raise ValueError(
    ...
    def _finalize_speaker(self, last_speaker: Agent, final: bool, name: str, agents: Optional[List[Agent]]) -> Agent:
        ...
        if len(mentions) == 1:
            ...
            logger.warning(
                f"Module select_speaker failed to resolve the next speaker's name. This is because the speaker selection OAI call returned:\n{name}"
            )
    ...
    def _auto_select_speaker(
        ...
        Args:
            last_speaker Agent: The previous speaker in the module
        ...
        # If no agents are passed in, assign all the module's agents
        if agents is None:
            agents = self.agents
        ...
        checking_agent = Agent("checking_agent", default_auto_reply=max_attempts)
    ...
    def _participant_roles(self, agents: List[Agent] = None) -> str:
        ...
        for agent in agents:
            if agent.description.strip() == "":
                logger.warning(
                    f"The agent '{agent.name}' has an empty description, and may not work well with Module."
                )
