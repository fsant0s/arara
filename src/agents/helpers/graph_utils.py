...
def check_graph_validity(
    ...
    Checks for the following:
        ...
        3. Every value is a list of BaseAgents (not string).
    ...
    # All values must be lists of BaseAgent or empty
    ...
    # Check 3. Every value is a list of BaseAgents or empty list (not string).
    if not all(
        [all([isinstance(agent, BaseAgent) for agent in value]) for value in allowed_speaker_transitions_dict.values()]
    ):
        raise ValueError("allowed_speaker_transitions_dict has values that are not lists of BaseAgents.")
    ...
    # allowed_speaker_transitions_dict.values() is a list of list of BaseAgents
    ...
    for key, values in allowed_speaker_transitions_dict.items():
        ...
        if unique_duplicates:
            logging.warning(
                f"BaseAgent '{key.name}' has duplicate elements: {[agent.name for agent in unique_duplicates]}. Please remove duplicates manually."
            )


def invert_disallowed_to_allowed(disallowed_speaker_transitions_dict: dict, agents: List[BaseAgent]) -> dict:
    """
