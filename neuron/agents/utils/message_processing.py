...
def process_message_before_send(
        self: Agent, message: Union[Dict, str], recipient: Agent, silent: bool
    ) -> Union[Dict, str]:
...
def process_received_message(
        self: Agent, message: Union[Dict, str], sender: Agent, silent: bool
    ) -> None:
    ...
    if not silent:
        #TODO: implement self.llm_config print_received_message(message, sender, self.name, self.llm_config)
        print_received_message(message, sender, self.name, None)

def process_last_received_message(self: Agent, messages: List[Dict]) -> List[Dict]:
    """
...
def process_all_messages_before_reply(self: Agent, messages: List[Dict]) -> List[Dict]:
    """
