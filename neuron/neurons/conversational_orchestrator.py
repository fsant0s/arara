...
class ConversationalOrchestrator:
    ...
    - allow_repeat_speaker: whether to allow the same speaker to speak consecutively.
        ...
        If `allow_repeat_speaker` is a list of Neurons, then only those listed agents are allowed to repeat.
    ...
    allow_repeat_speaker: Optional[Union[bool, List[Neuron]]] = None
    ...
    def __post_init__(self):
        ...
        if self.allowed_or_disallowed_speaker_transitions is None:
            ...
            # Else if self.allow_repeat_speaker is a list of Neurons, add self loops to the agents in the list
    ...
    def next_agent(self, agent: Neuron, agents: Optional[List[Neuron]] = None) -> Neuron:
    ...
    def select_speaker_msg(self, agents: Optional[List[Neuron]] = None) -> str:
    ...
    def select_speaker_prompt(self, agents: Optional[List[Neuron]] = None) -> str:
    ...
    def introductions_msg(self, agents: Optional[List[Neuron]] = None) -> str:
        """Return the system message for selecting the next speaker. This is always the *first* message in the context."""
        if agents is None:
            agents = self.agents
    ...
    def manual_select_speaker(self, agents: Optional[List[Neuron]] = None) -> Union[Neuron, None]:
    ...
    def random_select_speaker(self, agents: Optional[List[Neuron]] = None) -> Union[Neuron, None]:
    ...
    def _prepare_and_select_agents(
        self,
        last_speaker: Neuron,
    ) -> Tuple[Optional[Neuron], List[Neuron], Optional[List[Dict]]]:
        ...
        if isinstance(self.speaker_selection_method, Callable):
            ...
            if selected_agent is None:
            ...
            elif isinstance(selected_agent, Neuron):
                ...
                raise ValueError(
                    f"Custom speaker selection function returned an object of type {type(selected_agent)} instead of Neuron or str."
                )
    ...
    def select_speaker(self, last_speaker: Neuron, selector: Neuron) -> Neuron:
    ...
    def _finalize_speaker(self, last_speaker: Neuron, final: bool, name: str, agents: Optional[List[Neuron]]) -> Neuron:
        if not final:
    ...
    def _auto_select_speaker(
        ...
        # Neuron for checking the response from the speaker_select_agent
        checking_agent = Neuron("checking_agent", default_auto_reply=max_attempts)
        ...
        speaker_selection_agent = Neuron(
    ...
    def _participant_roles(self, agents: List[Neuron] = None) -> str:
    ...
    def _mentioned_agents(self, message_content: Union[str, List], agents: Optional[List[Neuron]]) -> Dict:
        """Counts the number of times each agent is mentioned in the provided message content.
        Neuron names will match under any of the following conditions (all case-sensitive):
        - Exact name match
        ...
        Args:
            ...
            agents (List[Neuron]): A list of Neuron objects, each having a 'name' attribute to be searched in the message content.
...
class ConversationalOrchestratorManager(Neuron):
    ...
    def _prepare_chat(
        ...
        if clear_history:
            self._chitchat.reset()

        for agent in self._chitchat.agents:
            if (recipient != agent or prepare_recipient) and isinstance(agent, Neuron):
    ...
    def clear_agents_history(self, reply: dict, chitchat: ConversationalOrchestrator) -> str:
        ...
        if agent_to_memory_clear:
            ...
            if nr_messages_to_preserve:
                ...
                # clearing history for chitchat here
                temp = chitchat.messages[-nr_messages_to_preserve:]
                chitchat.messages.clear()
                chitchat.messages.extend(temp)
            else:
