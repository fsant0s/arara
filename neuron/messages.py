...
class FunctionOrTextMessage(ChatMessage):
    """A message whose content can be either plain text or a FunctionCall.

    .. note::

        This message type supports flexible content formats, allowing agents to
        communicate either by sending direct natural language (string) or structured
        function call objects (:class:`FunctionCall`).

    This class is useful in situations where the agent may generate either
    plain text or a tool invocation request in the same message channel.

    Inherit from this class if your message content type can be a string or a FunctionCall.
    """

    content: str | List[FunctionCall]
    """The content of the message. Can be either a plain string or a FunctionCall object."""

    def to_text(self) -> str:
        """Convert the content to a string for display in the console or logs."""
        return str(self.content)

    def to_model_text(self) -> str:
        """Convert the content to a string for inclusion in model input."""
        return str(self.content)

    def to_model_message(self) -> UserMessage:
        """Convert the content to a :class:`UserMessage` for use with the model client."""
        return UserMessage(
...
class MessageFactory:
    ...
    def __init__(self) -> None:
        ...
        self._message_types[FunctionOrTextMessage.__name__] = FunctionOrTextMessage
        self._message_types
